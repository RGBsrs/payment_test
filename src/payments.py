...
def process_payment():
    ...
    if currency == "978":
        ...
        sign = generate_sign_for_payment(
            [data["amount"], data["currency"], data["shop_id"], data["shop_order_id"]]
        )
        data["sign"] = sign
        try:
            payment = Payment(
            ...
            db.session.flush()
            payment_logger(
                f"Time: {datetime.datetime.now()}, Currency: {data['currency']},Amount:{data['amount']}, Description {data['description']}, payment_id: {payment.id}"
            )
            db.session.commit()
        ...
        try:
            payment = Payment(
                currency_id=int(data["payer_currency"]),
            ...
            payment_logger(
                f"Time: {datetime.datetime.now()},Currency: {data['shop_currency']}, Amount:{data['shop_amount']}, Description {data['description']}, payment_id: {payment.id}"
            )
